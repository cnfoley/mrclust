...
#' @return Returned is a scatter plot of the two-stage association estimates for each variant in which: clusters are colour coded and variants with larger assignement/inclusion probabilities appear larger.
#' @export

two.stage.plot = function(res, bx, by, bxse, byse, obs.names){

  cbPalette <- c("Null" = "#CC79A7", "Junk" = "#000000", "1" = "#999999", "2" = "#0072B2", "3" = "#D55E00", "4" = "#F0E442", "5"  = "#009E73", "6"  = "#56B4E9", "7"  = "#E69F00");
  clst.class = unique(res$cluster.class);
  no.clst = clst.class[!clst.class %in% c("Null", "Junk")];
  if(!identical(no.clst, character(0))){
  n_palette = max(as.numeric(no.clst));
  }else{n_palette=0;}
  if(n_palette>=8){
    tmp.cbPalette = RColorBrewer::brewer.pal(max(3,n_palette), "Spectral")[8:n_palette];
    names(tmp.cbPalette) = paste0(8:n_palette);
    cbPalette = c(cbPalette, tmp.cbPalette);
  }

  tmp.orient = orientate(bx,by);
  bx = tmp.orient$bx; by = tmp.orient$by;

  # redefine null cluster as cluster zero
  res$cluster[res$cluster_mean == 0] = 0;
  res$clusters = as.factor(res$cluster);

  ord = match(res$observation, obs.names);
  res$bx = bx[ord];
  res$by = by[ord];
  res$bxse = bxse[ord];
  res$byse = byse[ord];
  ...
  p <- ggplot2::ggplot(data = res, ggplot2::aes(bx, by)) +
    ggplot2::geom_point(ggplot2::aes(colour = cluster.class, size = probability), shape = 21) +
    ggplot2::scale_color_manual(values = cbPalette) +
    ggplot2::labs(size = "Cluster\ninclusion\nprobability") +
    #+ guides(color=guide_legend(title= "Inclusion\nprobability")) +
    #scale_colour_gradient(low = "steelblue", high = "yellow") +
    #geom_point(aes(colour = probability)) +
    #scale_colour_gradient2() +
    ggplot2::geom_errorbarh(
      ggplot2::aes(xmin = bx-1.96*bxse, xmax = bx+1.96*bxse, color = cluster.class),
      #  width = 0.1,
      linetype = "solid") +
    ggplot2::geom_errorbar(
      ggplot2::aes(ymin = by-1.96*byse, ymax = by+1.96*byse, color = cluster.class),
      #  width = 0.1,
      linetype = "solid") +
    ggplot2::ylab("Genetic effect on outcome") + ggplot2::xlab("Genetic effect on exposure") +
  ...
  if(sum(ind)>0){
    for(i in 1:length(ind)){
      ...
      p = p + ggplot2::geom_abline(slope = mn, intercept = 0, color = col, linetype = "dotted", lwd=0.50);
    }
  }
  p = p + ggplot2::guides(color=ggplot2::guide_legend(title= "Cluster", labels = clust.label));
