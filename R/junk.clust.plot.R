junk.clust.plot = function(res, xrange = NULL, sig = NULL, mu = 0, mu.null = 0, junk.mixture = TRUE, null.mixture = TRUE){
  ...
  jit.null = dnorm(theta[ null.obs], mean = mu.null, sd = sig.null, log=F)
  jit.clusts = rep(0, length(theta));
  jit.clusts[junk.obs] = jit.junk*runif(sum(junk.obs), 0, 0.5);
  jit.clusts[null.obs] = jit.null*runif(sum(null.obs), 0, 0.5);
  if(sum(junk.obs)>0 | sum(null.obs)>0){mny = max(c(jit.clusts[junk.obs],jit.clusts[null.obs]));}else{mny = 1;}
  jit.clusts[clust.obs] = -runif(length(clust.obs), 0, mny);
  ...
  maxy = if(sum(junk.obs | null.obs)>0){
    ...
    if(sum(sig.null)>0){dnorm(0, 0, sig.null)}else{0};
  }else if(sum(junk.obs | null.obs)==0 & junk.mixture & null.mixture){
    max(max(gen.t(c(minx,maxx), mu = mu, sig = sig, log = FALSE)), if(sum(sig.null)>0){dnorm(0, 0, sig.null)}else{0});
  }
  ...
  mx.dens = if(junk.mixture & !null.mixture){
    ...
    max(gen.t(mu, mu = mu, sig = sig, log = FALSE),dnorm(0, 0, sig.null));
  }else{0}
