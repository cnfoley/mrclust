junk_clust_plot <- function(res, xrange = NULL, sig = NULL, 
  ...
  junk.obs <- res$cluster_class == "Junk"
  null.obs <- res$cluster_class == "Null"
  clust.obs <- which(!res$cluster_class == "Null" & !res$cluster_class == "Junk")
  theta <- res$theta
  ...
  if (sum(junk.obs) > 0 | sum(null.obs) > 0) {
    mny <- max(c(jit.clusts[junk.obs], jit.clusts[null.obs]))
  } else {
    mny <- 1
  }
  ...
  } else if (sum(junk.obs | null.obs) == 0 & junk_mixture & !null_mixture) {
    max(gen.t(c(minx, maxx), mu = mu, sig = sig, log = FALSE))
  } else if (sum(junk.obs | null.obs) == 0 & !junk_mixture & null_mixture) {
    if (sum(sig.null) > 0) {
      stats::dnorm(0, 0, sig.null)
    } else {
      0
    }
  } else if (sum(junk.obs | null.obs) == 0 & junk_mixture & null_mixture) {
    max(max(gen.t(c(minx, maxx), mu = mu, sig = sig, log = FALSE)), if (sum(sig.null) > 0) {
      stats::dnorm(0, 0, sig.null)
    } else {
      0
    })
  ...
  miny <- if (length(clust.obs) > 0) {
    min(jit.clusts[clust.obs])
  } else {
    0
  }
  mx.dens <- if (junk_mixture & !null_mixture) {
  ...
  p <- ggplot2::ggplot(data = res, ggplot2::aes(theta, jit.clusts)) +
    ...
    ggplot2::geom_errorbarh(
      ggplot2::aes(xmin = theta - 1.96 * theta.se, xmax = theta + 1.96 * theta.se, color = cluster_class),
      #  width = 0.1,
      linetype = "solid"
    ) +
  ...
  p <- p + ggplot2::geom_text(data = annotations, ggplot2::aes(x = xpos, y = ypos, hjust = hjustvar, vjust = vjustvar, label = annotateText), size = 3) +
    ggplot2::guides(color = ggplot2::guide_legend(title = "Cluster", labels = clust_label)) + ggplot2::xlim(minx, maxx)
  ...
  if (null_mixture & sum(null.obs) > 0) {
